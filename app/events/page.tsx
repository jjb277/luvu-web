...
export default async function EventsPage({
  ...
  let query = sb
    .from("events")
    .select("id, title, short_tagline, category, event_date, venue_name, city, base_price, primary_image_url, organizers(company_name)")
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, #2a3f40 0%, #1a2a2b 100%)" }}>
      ...
      <main className="max-w-6xl mx-auto px-6 pb-24">
        <div className="mb-8">
          ...
          <p className="text-sm" style={{ color: "rgba(232,240,240,0.45)" }}>{events?.length ?? 0} aankomende events in België</p>
        </div>
        ...
        <form method="GET" className="mb-6 flex gap-3">
          ...
          {(q || cat) && (
            <Link href="/events" className="px-4 py-2.5 rounded-xl text-sm" style={{ color: "rgba(232,240,240,0.5)" }}>
              Wis
            </Link>
          )}
        </form>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {CATEGORIES.map((c) => (
            <Link
              key={c}
              href={c === "Alle" ? "/events" : `/events?cat=${c}${q ? `&q=${q}` : ""}`}
        ...
        {!events?.length ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {events.map((e) => (
              <Link key={e.id} href={`/events/${e.id}`} className="rounded-2xl overflow-hidden flex flex-col group transition-transform hover:-translate-y-0.5"
                ...
                <div className="relative h-44 overflow-hidden" style={{ background: "#1a2a2b" }}>
                  {e.primary_image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-4xl">{CAT_ICONS[e.category ?? ""] ?? "🎭"}</div>
                  )}
